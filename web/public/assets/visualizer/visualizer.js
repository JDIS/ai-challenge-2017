...
function showGame(game, $container, maxWidth, maxHeight, showmovement, isminimal, offline, seconds) {
    ...
    if(!isminimal) {
        var $row = $("<div></div>");
        $row.append($("<div></div>").append($("<h4 style='margin-top: 0px;'>"+game.players.slice(1, game.num_players+1).map(function(p) {
    ...
    $container.append($("<span class='button button-primary' data-toggle='modal' data-target='#myModal'>Aide concernant le visualiseur</span> <div id='myModal' class='modal fade' role='dialog'> <div class='modal-dialog'> <div class='modal-content'> <div class='modal-header'> <button type='button' class='button button-primary' style='float:right;' data-dismiss='modal'>Fermer</button> <h4 class='modal-title'>Visualiseur de partie</h4> </div><div class='modal-body'> <ul><li>La barre d'espace permet de mettre la partie sur pause.</li><li>Les flèches Gauche et Droite permettent de naviguer dans le temps.</li><li>Les flèches Haut et Bas permettent de modifier la vitesse de lecture, tout comme les touches 1 à 5.</li><li>Les touches (+) et (-) permettent de zoomer sur le terrain.</li><li>Les touches Z et X permettent d'atteindre le début et la fin de la partie.</li><li>La touche P affiche la carte thermique de production.</li><li>La touche E permet d'afficher des informations supplémentaires.</li><li>Les touches W, A, S et D permettent de déplacer la vue. La touche O ramène à la vue par défaut.</li><li>Les touches Virgule et Point permettent de se déplacer dans le temps, un <em>frame</em> à la fois.</li><li>Il est aussi possible de cliquer sur les graphiques pour naviguer dans le temps.</li></ul></div></div></div></div>"));

    var showExtended = false;
    var frame = 0;
    var transit = 0;
    var framespersec = seconds == null ? 6 : game.num_frames / seconds;
    var shouldplay = true;
